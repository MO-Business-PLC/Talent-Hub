import React from "react";

export default function TopCompanies() {
  return (
    <section className="relative w-full">
      {/* Subtle top background image for depth */}
      <div
        className="absolute inset-0 pointer-events-none select-none"
        aria-hidden
        style={{
          backgroundImage: "url(/images/home/landing_page_top.jpg)",
          backgroundSize: "cover",
          backgroundPosition: "center",
          opacity: 0.08,
        }}
      />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 md:py-14">
        {/* Heading */}
        <div className="text-center mb-6 md:mb-8">
          <h2 className="text-xl md:text-2xl font-semibold text-[#1B2432]">
            Top Companies <span className="text-[#1E73BE]">Hiring</span> Now
          </h2>
          <p className="mt-2 text-sm md:text-[15px] text-[#6B7280]">
            Trusted by 120,000+ businesses worldwide to find top talent
          </p>
        </div>

        {/* Logos strip */}
        <div className="flex items-center justify-center mb-8 md:mb-12">
          <img
            src="/images/home/companies.png"
            alt="Trusted companies logos"
            className="max-w-full w-[980px] h-auto object-contain opacity-90"
            loading="lazy"
          />
        </div>

        {/* Stats cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
          <StatCard
            icon="/icons/home/briefcase.png"
            value="1,75,324"
            label="Live Job"
          />
          <StatCard
            icon="/icons/home/building.png"
            value="97,354"
            label="Companies"
          />
          <StatCard
            icon="/icons/home/avatar.png"
            value="38,47,154"
            label="Candidates"
          />
          <StatCard
            icon="/icons/home/briefcase.png"
            value="7,532"
            label="New Jobs"
          />
        </div>
      </div>
    </section>
  );
}

function StatCard({
  icon,
  value,
  label,
}: {
  icon: string;
  value: string;
  label: string;
}) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 md:px-6 py-4 md:py-5 flex items-center gap-3 md:gap-4">
      <div className="shrink-0 grid place-items-center w-10 h-10 md:w-12 md:h-12 rounded-xl bg-[#F5F7FB]">
        <img src={icon} alt="" className="w-5 h-5 md:w-6 md:h-6" />
      </div>
      <div>
        <div className="text-lg md:text-2xl font-semibold text-[#1B2432] leading-tight">
          {value}
        </div>
        <div className="text-[12px] md:text-sm text-[#6B7280]">{label}</div>
      </div>
    </div>
  );
}
